...
| PRE-016 | Repository layout, generated/committed content, ignored local state, large artifacts, stable IDs, formatting, dependency, supersession, and clean-clone rules are documented and checked. | Active | Proposed [repository hygiene contract](../project/repository-hygiene.md) awaiting owner acceptance; automated clean-clone check not yet implemented and its profile placement open |
