...
| PRE-012 | A concise agent context contract is documented, has explicit authority/update rules, and passes a clean-session comprehension exercise. | Pending | Context document and Codex/Claude/Copilot session results |
| PRE-013 | Canonical tool-neutral repository instructions, minimal tool adapters, task isolation, concurrent-edit, commit, review, and handoff rules are defined and loading-tested. | Pending | Repository guidance plus adapter/discovery test record |
