...
| PLN-0001-02 | complete | 01 | `src/slice/composition/mkosi.conf`, `src/slice/compose.sh`, and the [composition record](../project/slice-composition-record.md). 104-package closure from the single frozen repository; UKI, kernel, and initrd bit-identical across builds; disk image not reproducible | Complete 2026-08-10. Built unprivileged with no host mutation. Open: disk-image irreproducibility unexplained, tools tree fetched with `--nogpgcheck`, no registered check. PLN-0001-03 is next |
