...
| A-009 | bootc can satisfy the eventual NeutrinOS trust and failed-boot requirements on a supportable production backend. | Accept the threat and trust requirements, then exercise signed update, boot assessment, fallback, rollback, and recovery without relying on an unaccepted experimental path. |
