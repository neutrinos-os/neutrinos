...
| A-010 | Manual platform signing and release authorization are operable for the initial personal fleet, including an urgent release. | Owner confirmation followed by a timed disposable-key promotion, emergency gate, and signer-replacement exercise. |
| A-011 | The maintainer can retain an offline authority recovery copy or succession path outside the primary local-disaster and normal-account failure domains. | Owner feasibility review followed by a private custody worksheet and non-destructive retrieval exercise. |
