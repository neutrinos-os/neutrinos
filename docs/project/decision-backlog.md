...
| P-009 | Which VM runner provides qualification evidence beyond the first slice, and what may be taken from GPL-3.0 tooling? | Open: QEMU is a fixture chosen without comparison. It alone provides a writable firmware varstore and a TCG fallback; cloud-hypervisor and test.thing provide neither, and test.thing's GPL-3.0-or-later licence is incompatible with copying into this Apache-2.0 repository. Depends on `W-002`: if NeutrinOS ships a VMM it stops being a harness and becomes a declared input. See [RES-0013](../research/comparisons/vm-test-harness.md) | P-004, P-007, W-002 |
