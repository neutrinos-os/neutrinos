...
| 9 | Is the tools-tree reuse acceptable, and should the slice adopt it rather than keeping two build roots | **Ruled 2026-08-12 by Jason Tarasovic: consolidate on the slice tree.** The spike's stated reason for a separate tree was the missing `createrepo_c`, and PLN-0002-02 added it to the slice recipe, so the two declarations are now byte-identical in package list and pinned base digest. One tools tree, one build root. The reuse that prompted this was never verified byte-for-byte because the repository was unreachable; it is reachable now, so consolidation is confirmed by a rebuild rather than by the declaration alone. **Implemented and confirmed 2026-08-12.** The trees were first measured directly: identical file lists at 16293 entries, identical package sets at 234 NEVRAs, and nine differing files that are all nondeterministic build metadata. `spike.sh` then dropped its own tools-tree build and took the slice's, and the artifact was rebuilt through it: `/usr`, the verity tree, the verity signature, the root hash, the UKI, the initrd and the manifest are all **byte-identical** to the retained artifact. The ESP and the whole-disk image differ, and a control rebuild -- the same script run twice -- differs in the same way, so the difference is the build's own nondeterminism and not the tools tree. That nondeterminism is a finding against PLN-0002-07's build-determinism criterion rather than against this question, and is recorded in the [current context](current-context.md) |
