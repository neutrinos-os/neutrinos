...
| --- | --- | --- | --- | --- | --- |
| Repository, agent, context, and test readiness ([PLN-0000](../plans/0000-pre-implementation-readiness.md)) | Accepted execution governance; not product architecture | Root `AGENTS.md` with thin Claude/Copilot adapters; test entry point and repository conventions open | Manual checks and agent semantics/adversarial probes pass; [EX-0016](../research/exercises/0016-agent-context-and-instruction-loading.md) `8dd0669` rerun failed Codex context budget and found stale next action; Copilot unverified | Active | Blocking G1; structurally repair/rerun context route, then define test layers, repository baseline, and housekeeping |
