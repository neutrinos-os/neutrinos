...
| Authenticated `/usr` artifact format comparison ([PLN-0002](../plans/0002-usr-artifact-format-spike.md), **complete**) | Bounded by the same mutation boundary, ADR-0001, ADR-0003, DES-0006 and DES-0005 as amended 2026-08-11; selects no ecosystem, layout, or mechanism and does not accept its own recommendation | None selected. EROFS and ext4 are the two compared `/usr` candidates; the confext carve and tooling task 03a drew are candidate and hand back to DES-0005 and ADR-0003 | **Complete, accepted 2026-08-15.** Tasks 01, 02 complete; 03a and 04 partial; 05 accepted 2026-08-12, implemented, and audited against the built artifacts 2026-08-14 with three corrections taken and its two open parameters ruled; 06 complete and accepted 2026-08-14, six artifacts built and digests retained ([artifact set](usr-artifact-set.md)); 07 complete and accepted 2026-08-15, five of C-007's eight criteria measured offline ([measurements](artifact-format-measurements.md)); 08 complete and accepted 2026-08-15, boot behaviour and memory measured over three boots per arm and both are ties ([boot records](artifact-boot-records.md)); 09 complete and accepted 2026-08-15, corruption behaviour measured over four injections and it is the **first criterion to separate the arms** ([corruption records](artifact-corruption-records.md)); 10 complete and accepted 2026-08-15, seven cells per arm over 32 boots, image substitution failing closed and signature substitution failing open on every cell ([substitution records](artifact-substitution-records.md)), which completes every measurement task; 11 complete and accepted 2026-08-15, the five registered slice tests audited true on both arms with `T3-SLICE-004` and `T4-SLICE-002` registered and `T2-SLICE-002` widened to the comparison's own premise ([check updates](slice-check-updates.md)), and, on the owner's ruling of the same date, `T4-SLICE-003` and `T4-SLICE-004` registered **deferred** so task 10's `/usr` signature fail-open keeps a registered obligation after this plan closes; 12 **complete and accepted 2026-08-15** ([disposition](artifact-recovery-disposition.md)), splitting the recovery criterion into a measured format layer -- eight injection sites, a tie on file data and **ext4 ahead on metadata diagnosis**, where `fsck.erofs` detects a corrupt superblock and exits 0 anyway and sees nothing below it -- and a system layer deferred to verification items 3 and 5 as an accepted amendment to item 2, resolving `crypttab` as unsatisfiable because the artifact contains none, and owing PLN-0002-07 a correction -- `fsck.erofs --extract=X --path=<file>` writes to `X` itself and does not fail open. Task states and standing findings are in the [current context](current-context.md); detail in the [early-boot record](spike-early-boot-record.md), [composition record](usr-artifact-composition.md), [carve record](etc-path-carve.md), and [parameter declaration](artifact-parameter-declaration.md); 13 complete and accepted 2026-08-15, recommendation and weighing rule both -- **EROFS, conditional on the update mechanism not being whole-image-only** ([recommendation](artifact-format-recommendation.md)), which **does not accept EROFS**; 14 **complete and accepted 2026-08-15** ([evidence bundle and DES-0006 disposition](artifact-evidence-bundle.md)): bundle retained outside the repository at 9343 KiB across 130 files with a clean unsafe-output scan, the plan's requirement trace updated from planned evidence to observed results, and the exit-criteria assessment accepted with its qualifications on criteria 1 and 5 -- **SYS-049's substitution clause is measured half met**, holding for the image and failing for the signature. **03b moved out of the plan by the same ruling** (amendment 6): confext delivery is an open sub-question under `S-004`, owned by DES-0005, and PLN-0002-04's unplaced confext partition travels with it | Complete, VM-only; no implementation authority survives the plan. `check:complete` run green at 16 of 16 on 2026-08-15 against the EROFS primary, after PLN-0002-11 registered two checks; 18 registered, 2 of them deferred and never selected. Both profiles are retained in the PLN-0002-14 bundle: `fast` 8/0, `complete` 16/0 with 2 deferred | G1 satisfied. **Accepted complete by Jason Tarasovic 2026-08-15**, with two qualifications in its assessment: **03a and 04 stay partial permanently**, their remainders having left with 03b, and no registered check re-measures a figure the plan produced. **Completion ends the implementation authority**: PLN-0000's boundary needs G1 plus an accepted follow-on plan, and there is none. **Next action: an ADR on C-007, or a plan for DES-0006 verification items 3, 4 and 5.** C-007 stays open until an ADR records a format. Four of C-007's eight criteria separate the formats not at all. **03b left the plan 2026-08-15** by owner ruling, after being deferred rather than sequenced ahead of the measurement tasks on 2026-08-14; nothing depended on it. **One owner ruling still qualifies it**: the ParticleOS command-line ruling of 2026-08-12, unimplemented and contradicted by the accepted declaration's own measured argument. Three others were settled 2026-08-14 -- amendment 5 **accepted**, so 06 builds six artifacts; `systemd.image_filter=` **ruled absent**, the premise making it load-bearing having assumed two artifacts visible to one boot when task 10 substitutes the disk; and the initrd module list **accepted as measured, not tightened**, since no C-007 criterion needs the trim and kernel content is `W-004`. No parameter inside the signed UKI is open. Standing risks: PR-0030 C-006, and PR-0029 C-005 |
| `/usr` read-workload comparison ([PLN-0003](../plans/0003-usr-read-workload-comparison.md)) | Bounded by the same mutation boundary; claims no requirement, accepts no format, and records no decision | None selected. EROFS and ext4 remain the two candidates; the two workload shapes and the memory cells are declared fixtures with no role contract behind them | Active. Accepted 2026-08-15 after [PR-0031](reviews/0031-usr-read-workload-comparison-plan.md) found the first draft not fit to accept and its ten challenges were ruled the same day. Measures what a wide read across `/usr` costs on each format -- PLN-0002-13's threat 1, and the one result that could reverse the C-007 recommendation. Task 01, the declaration, is next | Active, VM-only. The six PLN-0002-06 artifacts are **read-only inputs**, `snapshot=on` with digests verified before use; **non-member** artifacts may be built for attribution and instrument control, marked as such and destroyed at task end | G1 satisfied; implementation authorized only within this plan's disposable-VM scope. **Two risks are accepted with it**: no reversal threshold is pre-registered, so the weighing happens with the figures visible (C-001), and the workload fixtures become the reference by being first (C-008). Schedule bound: the synthetic signing material **expires 2026-09-11**. C-007 stays open until an ADR records a format |
...
PLN-0000 complete 2026-08-10 (PRE-001 through PRE-018 satisfied)
  ...
  -> PLN-0002-13 accepted 2026-08-15: EROFS recommended, conditionally, on image
     size; the acceptance covers the recommendation and its rule, not EROFS
  -> PLN-0002-14 complete 2026-08-15, open for owner decision: evidence bundle
     retained, requirement trace updated to observed results, DES-0006
     disposition accepted with it. The same ruling moved 03b out of the plan to
     an open sub-question under S-004, which met exit criterion 1
  -> PLN-0002 accepted complete 2026-08-15: no active plan, so no implementation
     authority until the next one is accepted
  -> PLN-0003 accepted 2026-08-15: the /usr read-workload comparison, which is
     PLN-0002-13's threat 1 and the one measurement that could reverse EROFS
  -> NOW: PLN-0003-01, the workload and measurement declaration
  -> then an ADR on C-007, and a plan for DES-0006 items 3, 4 and 5
