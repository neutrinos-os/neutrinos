...
| Repository change flow and CI ([P-008](decision-backlog.md), [P-009](decision-backlog.md), [P-010](decision-backlog.md)) | Open on all three | QEMU is a fixture chosen without comparison ([RES-0013](../research/comparisons/vm-test-harness.md)); no corpus-maintenance mechanism selected ([RES-0016](../research/comparisons/record-corpus-maintenance.md)) | RES-0013 and RES-0016 complete | Validation runner and both canonical profiles implemented | Non-blocking under G1. **`main` is pushed and current**, with owner bypass enabled as a deliberate temporary state, so each push lands recording `Bypassed rule violations`. `P-008`'s constraint -- answer it **before** the CI `complete` job runs anywhere, since a hosted runner has no composed artifact -- was **already violated when it was set** on 2026-08-12, and the `complete` job has now failed 39 consecutive runs. The default being taken is that a red `complete` is accepted. `P-010` is deliberately left open until after G2, with its observed failure rate as the accepted cost |
...
PLN-0000 complete 2026-08-10 (PRE-001 through PRE-018 satisfied)
  ...
  -> NOW: PLN-0002-07, measuring the eight criteria the comparison turns on
