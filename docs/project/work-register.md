...
| Authenticated `/usr` artifact format comparison ([PLN-0002](../plans/0002-usr-artifact-format-spike.md)) | Bounded by the same mutation boundary, ADR-0001, ADR-0003, DES-0006 and DES-0005 as amended 2026-08-11; selects no ecosystem, layout, or mechanism and does not accept its own recommendation | None selected. EROFS and ext4 are the two compared `/usr` candidates; the confext carve and tooling task 03a drew are candidate and hand back to DES-0005 and ADR-0003 | Active. Tasks 01, 02 complete; 03a and 04 partial; 05 accepted 2026-08-12 and fully implemented; 06 in progress; 10 started out of order with one cell covered. Task states and standing findings are in the [current context](current-context.md); detail in the [early-boot record](spike-early-boot-record.md), [composition record](usr-artifact-composition.md), [carve record](etc-path-carve.md), and [parameter declaration](artifact-parameter-declaration.md) | Active, VM-only. `check:complete` passes as a gate at 14 checks | G1 satisfied; implementation authorized only within this plan's disposable-VM scope. **Next action: the remainder of PLN-0002-06** -- build the artifacts and retain digests. **Two owner rulings block or qualify it and neither is an agent's to settle**: how many artifacts task 06 builds (amendment 5 drafted, unaccepted, six against the plan text's four), and the ParticleOS command-line ruling of 2026-08-12, which is unimplemented and contradicted by the accepted declaration's own measured argument. Standing risks: PR-0030 C-006, and PR-0029 C-005 |
...
| Repository change flow and CI ([P-008](decision-backlog.md), [P-009](decision-backlog.md), [P-010](decision-backlog.md)) | Open on all three | QEMU is a fixture chosen without comparison ([RES-0013](../research/comparisons/vm-test-harness.md)); no corpus-maintenance mechanism selected ([RES-0016](../research/comparisons/record-corpus-maintenance.md)) | RES-0013 and RES-0016 complete | Validation runner and both canonical profiles implemented | Non-blocking under G1 and nothing is pushed. `P-008` must be answered **before** the CI `complete` job runs anywhere, since a hosted runner has no composed artifact; explicitly punted 2026-08-12. `P-010` is deliberately left open until after G2, with its observed failure rate as the accepted cost |
...
PLN-0000 complete 2026-08-10 (PRE-001 through PRE-018 satisfied)
  ...
  -> NOW: PLN-0002-06, building the artifacts the comparison measures
