...
| --- | --- | --- | --- | --- | --- |
| Repository, agent, context, and test readiness ([PLN-0000](../plans/0000-pre-implementation-readiness.md)) | Accepted execution governance; not product architecture | Root `AGENTS.md` with thin Claude/Copilot adapters; test entry point and repository conventions open | Manual documentation checks pass; [EX-0016](../research/exercises/0016-agent-context-and-instruction-loading.md) Codex/Claude initial run passes semantics and adversarial probes; reading-scope repair/rerun pending; Copilot unverified | Active | Blocking G1; close EX-0016, then define test layers, repository baseline, and housekeeping before activating implementation |
