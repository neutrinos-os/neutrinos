...
| --- | --- | --- | --- | --- | --- |
| Repository, agent, context, and test readiness ([PLN-0000](../plans/0000-pre-implementation-readiness.md)) | Accepted execution governance and [test and evidence strategy](test-strategy.md); not product architecture | Root `AGENTS.md`, hard-bounded current context, thin Claude/Copilot adapters; T0-through-T7 levels, `T5@Tn` failure notation, and trace contract accepted; validation entry point and repository conventions open | [EX-0016](../research/exercises/0016-agent-context-and-instruction-loading.md) passed Codex/Claude semantics, adversarial probes, freshness, discovery, and cold routing at `c96fdbb`; [PR-0017](reviews/0017-test-strategy.md) accepted; Copilot owner-deferred and unverified | Active | Blocking G1; PRE-012 through PRE-014 satisfied; define PRE-015 validation commands and execution contract, then repository baseline and housekeeping |
...
```text
complete PLN-0000 agent context, repository, and test readiness tasks
  -> accept a bounded reference-VM implementation plan
  -> G1: implement only in disposable VM/lab scope
