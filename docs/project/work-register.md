...
| --- | --- | --- | --- | --- | --- |
| Repository, agent, context, and test readiness ([PLN-0000](../plans/0000-pre-implementation-readiness.md)) | Accepted execution governance; not product architecture | Root `AGENTS.md`, hard-bounded current context, thin Claude/Copilot adapters; test entry point and repository conventions open | Manual checks and agent semantics/adversarial probes pass; [EX-0016](../research/exercises/0016-agent-context-and-instruction-loading.md) `e3be851` rerun passed Claude routing while Codex still over-read PLN-0000 and ran unnecessary checks; final repair pending rerun; Copilot unverified | Active | Blocking G1; run one final Codex/Claude evaluation, record pass or bounded Codex limitation without further routing iteration, then define test layers, repository baseline, and housekeeping |
